...
BOOL cagdToObjectAtDepth(int x, int y, CAGD_POINT p, CAGD_POINT* where)
{
    GLdouble X ,Y;
    GLdouble winZ;

    // Get the depth value (winZ) of point p in the viewer's coordinate system
    gluProject(p.x, p.y, p.z, modelView, projection, viewPort, &X, &Y, &winZ);
    X = x;
    Y = viewPort[3] - y;
    // Get the point on the viewer screen plane where the user clicked
    BOOL success1 = gluUnProject(X, Y, 0.0, modelView, projection, viewPort, &where[0].x, &where[0].y, &where[0].z);

    // Get the point on the plane parallel to the viewer screen that contains point p
    BOOL success2 = gluUnProject(X, Y, winZ, modelView, projection, viewPort, &where[1].x, &where[1].y, &where[1].z);
...
void drawAxis()
{
   CAGD_POINT X_axis[] = { {0,0,0} ,{0x7ff0000000000000,0,0} };
   CAGD_POINT Y_axis[] = { {0,0,0} ,{0,0x7ff0000000000000,0} };
   CAGD_POINT Z_axis[] = { {0,0,0} ,{0,0,0x7ff0000000000000} };
   cagdSetColor(255, 0, 0);
   cagdAddPolyline(X_axis, sizeof(X_axis) / sizeof(CAGD_POINT));
   cagdSetColor(0, 255, 0);
   cagdAddPolyline(Y_axis, sizeof(Y_axis) / sizeof(CAGD_POINT));
   cagdSetColor(0, 0, 255);
   cagdAddPolyline(Z_axis, sizeof(Z_axis) / sizeof(CAGD_POINT));
}
...
void scalePoint(GLdouble factor)
{
...
void update_timer_speed(int uEpsilon) {
    timer_speed = uEpsilon;
}
int get_timer_speed() {
    return timer_speed;
}